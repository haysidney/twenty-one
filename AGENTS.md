...
- `BeginPlayerTurns` and `AdvanceToNextPlayer` auto-skip consecutive BJ hands. `BeginPlayerTurns` checks if all remaining active hands are BJ; if so, transitions directly to `DealerTurn` (or `Payout`) with no narration. If a non-BJ hand follows (mixed case), narrates "moving along" for the first BJ and sets `WaitingForNextPlayer=true` (old behavior). `AdvanceToNextPlayer` applies the same scan-and-skip logic when encountering BJ hands mid-round. The `allTerminalWin` check in `CanGoToPayout` ensures mixed BJ+Charlie hands also skip the dealer turn when appropriate.
