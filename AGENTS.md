...
- `BjPayout` is a free-form `double` multiplier (e.g. 1.5 = 3:2, 1.2 = 6:5, 1.0 = even money). Canonical on `VenueSettings`, mirrored on `GameState` so it is snapshotted with each undo entry. Edited via the `config.BjPayout` proxy (writes to the active venue); seeded into the live `GameState` by `Configuration.SeedRulesIntoGameState()` on every `NewRound`. Mid-round edits do not affect the running round. Engine payout uses `Math.Ceiling(bet * (decimal)state.BjPayout)`. CharliePayout remains an enum (3:2 / 6:5 / 1:1) since each value requires a full solver re-solve.
